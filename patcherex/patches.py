...
class InlinePatch(Patch):
    def __init__(self, instruction_addr, new_asm, name=None, force_consistent_size=True):
        super(InlinePatch, self).__init__(name)
        self.instruction_addr = instruction_addr
        self.new_asm = new_asm
        self.force_consistent_size = force_consistent_size
...
class InsertCodePatch(CodePatch):
    def __init__(self, addr, code, name=None, is_att=False, priority=1, stackable=False, force=False):

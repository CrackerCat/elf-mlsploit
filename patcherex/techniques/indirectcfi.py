...
class IndirectCFI(object):
    ...
    def handle_standard_cj(self,instruction,ff):
        ...
        if instruction.mnemonic == u"call":
            gadget_protection = '''
            ; less significant byte of target in dl (and check access)
            mov dl, BYTE [edx]

            ; prevet call to pop
            cmp dl,0x58
            jb _gadget_protection_good_{global_counter}
            cmp dl,0x5f
            ja _gadget_protection_good_{global_counter}
            
            hlt

            _gadget_protection_good_{global_counter}:
            '''.format(
                global_counter=IndirectCFI.global_counter
            )
        else:
            gadget_protection = ""
        ...
        if not self.allocate_executable:
            ...
            call _region_protection_get_pc_{global_counter}
            _region_protection_get_pc_{global_counter}:
            pop ecx

            sub ecx, OFFSET _region_protection_get_pc_{global_counter}
            cmp edx, ecx
            jb _region_protection_bad_{global_counter}

            ; TODO: use section info to set limit (highest_section_addr + sizeof(highest_section))
            ; assume binary size < 16MB
            add ecx, 0x1000000
            cmp edx, ecx
            ja _region_protection_bad_{global_counter}

            jmp _region_protection_good_{global_counter}
            _region_protection_bad_{global_counter}:
            hlt
            _region_protection_good_{global_counter}:
            nop
            '''.format(
                global_counter=IndirectCFI.global_counter
            )
        ...
        if not self.allocate_executable:
            # function entries have 0x10 alignment
            target_alignment_check = '''
            test edx, 0xf
            jnz _target_alignment_check_bad_{global_counter}

            jmp _target_alignment_check_good_{global_counter}
            _target_alignment_check_bad_{global_counter}:
            hlt
            _target_alignment_check_good_{global_counter}:
            nop
            '''.format(
                global_counter=IndirectCFI.global_counter
            )
        ...
        # helpful if NX is not available
        if not self.allocate_executable:
            target_elf_check = '''
            mov ecx, edx
            _target_elf_check_continue_search_{global_counter}:
            and edx, 0xfffff000
            ; no ELF header -> crash
            cmp DWORD [edx], 0x464c457f
            sub edx, 0x1000
            jnz _target_elf_check_continue_search_{global_counter}
            mov edx, ecx
            '''.format(
                global_counter=IndirectCFI.global_counter
            )
        ...
        if self.is_mainbin_call(instruction.address,ff):
            ...
            '''.format(
                ...
                gadget_protection=gadget_protection,
                target_region_protection=target_region_protection,
                target_alignment_check=target_alignment_check,
                target_elf_check=target_elf_check
            )
    ...
    def contains_executable_allocation(self,cfg):
        ...
        for bb_addr in allocate_callers:
            ...
            if not isx_flag.symbolic:
                if succ.state.se.eval(isx_flag) == 1:

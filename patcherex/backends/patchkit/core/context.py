...
class Context(object):
    def __init__(self, binary, backend, verbose=False):
        self.binary = binary
    ...
    def asm(self, asm, addr=0):
        return self.arch.asm(asm, addr=addr)
    ...
    def make_writable(self, addr):
        # Don't silently make our segment writable
    ...
    def hook(self, src, dst, first=False, noentry=False):
        ...
        # TODO: self.alloc()?
        stage1_addr = self.binary.alloc(len(stage0), target='code')
        stage2_addr = self.binary.alloc(len(stage0), target='code')
    ...
    def inject(self, **kwargs):
        ...
        c = kwargs.get('c')
        if c:
            asm = compiler.compile(c, self.binary.linker, arch=self.arch)
            raw = self.asm(asm, addr=addr)
            typ = 'c'
            is_asm = True
        else:
    ...
    def patch(self, addr, **kwargs):
        addr = self.arch.normalize_addr(addr)
        raw, typ = self._compile(addr, **kwargs)
        desc = kwargs.get('desc', '')
        if desc:
            desc = ' | "%s"' % desc

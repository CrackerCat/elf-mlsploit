...
def compile(code, linker, syms=(), arch=None):
    global compiler_version
    cflags = ['-Os', '-std=c99', '-fno-pic', '-ffreestanding', '-fno-stack-protector']

    if arch is not None:
        cc = arch.cc

    if compiler_version is None:
        compiler_version = subprocess.check_output([cc, '--version'])
    ...
    p = subprocess.Popen([cc, '-xc', '-S', '-o-', '-'] + cflags, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

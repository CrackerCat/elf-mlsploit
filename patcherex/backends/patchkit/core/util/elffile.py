...
EM('EM_AARCH64', 183, 'ARM 64-bit architecture (AARCH64)')
...
class ElfFile(StructBase):
    ...
    def _pack_program_data(self, block, offset=0):
        ...
        # FIX: Overwrite segments with loadable segments
        #
        # Segments could be overlapped,
        # e.g., LOAD & DYNMAIC
        # LOAD           0x000eec 0x00001eec 0x00001eec 0x00138 0x0013c RW  0x1000
        # DYNAMIC        0x000ef8 0x00001ef8 0x00001ef8 0x000f0 0x000f0 RW  0x4
        #
        # Even we update the LOAD segment using write(), the DYNAMIC segment
        # will contain the old value, and the final binary will not be updated
        # due to the old value. Therefore, we use data in the LOAD segment,
    ...
    def write(self, vaddr, data):
        ...
        if progs:
            ...
            if len(data) > ph.memsz - off:
                ph.memsz += len(data) - ph.memsz + off
                ph.filesz += len(data) - ph.filesz + off
                ph.size +=  len(data) - ph.memsz + off

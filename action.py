    ...
    for line in objdump.split("\n"):    
        if line != "":
            if extract_call_name(line, ignore_indirect=True) != NONE:
                ...
                if extract_call_name(line, ignore_indirect=True) == ADDR:
                  ext_addr = extract_org_addr(line)
                  if ext_addr != "0x0":
                    caller_callee["0x"+line.split(":")[0].strip()] = ext_addr
    return call_list, addr_list, caller_callee
...
def dump_addr(cand_dict, pn):
  ...
  with open(pn, 'wb') as f:
    pickle.dump(cand_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
...
def patch_bin(in_pn, out_pn, PATCH_ADDR_FILE, limitnum="True"):
  print "checking the addr file"
  os.system("ls -al %s" % PATCH_ADDR_FILE)
  cmd = "python patchbin.py %s %s %s %s" % (in_pn, out_pn, PATCH_ADDR_FILE, limitnum)
  os.system(cmd)
...
class Classifier(object):
  ...
  def classify_all(self, json_file):
    ...
    with open(json_file, 'r') as fh:
      ...
      for i in xrange(len(filelist)):
        ...
        self.log( "  >> classified as %s. This is %s result" % (result.upper(), status.upper()))        
  ...
  def perturb_candidate(self, json_file):
    ...
    with open(json_file, 'r') as fh:
      ...
      for i in xrange(len(filelist)):
        ...
        if result == "ben":
          self.log( "  >>> doesn't necessary to perturb %s" % filelist[i])
          continue
        ...
        sortedlist = get_highest_negdiff_feature(closest_benign, perturbed_array, 100)        
        
        self.log( " >>> should minimize this index")
        for item in sortedlist:          
          two_gram = ret_twogram_from_idx(self.two_gram, item[0])
          self.log( "idx %d, value %d, two-gram: %s" % (item[0], item[1], two_gram))
        ...
        patch_candidate, caller_callee = self.extract_addr(filelist[i], pert_candidate)        
        #dump_addr(patch_candidate, PATCH_ADDR_FILE)
        ...
        if os.path.getsize(out_pn) > 0:
          tag_dict = {"tag_a":filelist[i], "tag_b":str(os.path.getsize(out_pn))}
          status = "success"
  ...
  def gen_perturb_one(self, in_pn, out_pn):
    ...
    while True:
      ...
      self.log( " >>> perturbing %dth index (current feature diff %d)" % (target_perturb_idx, diff))
  ...
  def save_model(self, out_pn):    
    ...
    self.result.append ({"tag_a":"%s classifier" % self.model, "tag_b":"NP"})
...
def main():
  ...
  if name == "tra":
    ...
    cf = Classifier(in_dir, crossval=False, action="", name=name, model=model_file)
    cf.log( "[*] processing training")
    cf.log( " >>> user specified %s algorithm" % algorithm)
    cf.log( " >>> module will store model at: %s" % model_file)
    cf.train(model=algorithm, argument = argu_dict[algorithm])

    # assume that feature_mlsploit.py collectly generate two_gram_mini.pkl (/mnt/output/)
    cf.save_model(os.path.join(out_dir, model_file))
    cf.dump_json(output_json, input_json, null_file = True)
    cf.ret_report()
    cf.dump_log(output_global_log)
    ...
    cf = Classifier(in_dir, crossval=False, action="", name=name, model=model_file)
    cf.log( "[*] processing evaluation")
    cf.log( " >>> user specified model at: %s" % model_file)
    cf.load_model(os.path.join("/mnt", "input", model_file))
    cf.classify_all(input_json)
    cf.dump_json(output_json, input_json)
    cf.dump_log(output_global_log)
    ...
    cf = Classifier(in_dir, crossval=False, action="", name=name)
    cf.load_model(os.path.join("/mnt", "input", model_file))
    cf.perturb_candidate(input_json)
    cf.dump_json(output_json, input_json)
    cf.dump_log(output_global_log)
